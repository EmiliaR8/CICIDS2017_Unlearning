...
rationale: time-based tasks, the poisoning mechanic, category diversity floor, per-task
test subbatches, per-task accuracy matrix + pooled/mean accuracy, category-sliced confusion
matrices). The ONLY substantive difference from naive, matching clmd_ember18_joint_cd.py's
relationship to clmd_ember18_naive_cd.py:

  - Naive fine-tunes the SAME model incrementally, training only on each task's own new data.
  - Joint POOLS every task seen so far (0..tid) and trains a FRESH model from scratch on that
    pool each round -- the "what if we could afford to fully retrain every time" oracle
    baseline. Everything else (data construction, evaluation methodology, scaler discipline)
    is identical between the two files so results are directly comparable.

Run from the repo root (needs red_agent_perturbed_dataset.pkl under DATASET_PATH):
    python clmd_cicids_joint_cd.py --seed 42 --log_name joint_run1
"""
...
# --- CLI ---
parser = argparse.ArgumentParser(description="Continual Learning Experiment (CICIDS Joint Retraining Oracle)")
parser.add_argument('--seed', type=int, default=42, help='Random seed for reproducibility')
parser.add_argument('--log_name', type=str, default='cicids_joint_run',
...
MIN_CATEGORY_COUNT = 5              # floor per category, per split
...
def assign_task_windows(df: pd.DataFrame, num_tasks: int, task0_fraction: float) -> pd.DataFrame:
    ...
    TIMESTAMP (true chronological order -- source_row_id/orig_row_id are per-day counters
    that reset to 0 for each source_range, so sorting by them directly would interleave rows
    from different days by coincidence of row number instead of time). Task 0 gets
    task0_fraction of rows; the rest split evenly."""
...
def assign_perturbed_task_ids(df: pd.DataFrame, originals_with_task: pd.DataFrame) -> pd.DataFrame:
    ...
    placement -- no separate poisoning schedule). orig_row_id/source_row_id are only unique
    WITHIN one day's source file (they reset to 0 per source_range), so a multi-day dataset
    needs (source_range, row_id) as the join key -- row_id alone collides across days."""
...
def train_epochs(model, X, y, epochs, batch_size, active_count, device):
...
if __name__ == "__main__":
    ...
    # NOT refit as more tasks are pooled in below -- joint could legitimately refit on all
    # seen data each round, but the INPUT REPRESENTATION must stay shared with naive/MADAR/
    # unlearning for a fair comparison (same reasoning as clmd_ember18_joint_cd.py). ---
    ...
    # --- Training (JOINT: pool tasks 0..tid, fresh model every round) + per-task-matrix eval ---
    seen_labels = set()
    ...
    for tid in range(NUM_TASKS):
        # JOINT: pool every task's train split seen so far (0..tid) and train a FRESH model
        # from scratch on the pool -- the "full retrain every task" oracle baseline.
        Xtr = torch.cat([tasks[j]["train_tensors"][0] for j in range(tid + 1)], dim=0)
        ytr = torch.cat([tasks[j]["train_tensors"][1] for j in range(tid + 1)], dim=0)

        seen_labels.update(np.unique(ytr.numpy()).tolist())
        active_count = len(seen_labels)

        print(f"\n=== Training Task {tid} (joint retrain on tasks 0..{tid}) | "
              f"pooled_train_samples={len(Xtr)} | active_count={active_count} ===")

        model = ClassifierNN(INPUT_DIM, num_classes).to(DEVICE)  # fresh model every round
        grad_steps = train_epochs(model, Xtr, ytr, EPOCHS_PER_TASK, BATCH_SIZE, active_count, DEVICE)
    ...
    # --- Save log + plot ---
    log_path = f"{LOG_NAME}_joint_Task0Clean_{TASK0_CLEAN_ONLY}_{AGENT_MODE}.json"
    ...
    plt.title(f'CICIDS Joint Retraining Oracle ({LOG_NAME}, seed {SEED})')
    plt.legend(); plt.grid(alpha=0.3)
    plt.savefig(f"{LOG_NAME}_joint_Task0Clean_{TASK0_CLEAN_ONLY}_{AGENT_MODE}.png")
