...
def assign_task_windows(df: pd.DataFrame, num_tasks: int, task0_fraction: float,
    ...
    """Sliced/sorted by TIMESTAMP (true chronological order) -- source_row_id/orig_row_id
    are per-day counters that reset to 0 for each source_range, so sorting/windowing by them
    directly would interleave rows from different days by coincidence of row number instead
    of time."""
