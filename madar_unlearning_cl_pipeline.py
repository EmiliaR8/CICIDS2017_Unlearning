...
# ---------------------------------------------------------------------------
# Decision-boundary visualization (NEW, identical to madar_cl_pipeline.py's
# copy of the same section -- see its comments for the full rationale).
# Evaluates the CLASSIFIER itself over a dense grid -- an actual decision
# surface, not a data embedding -- on a FIXED run-wide 2D PCA plane
# (pca_mean/pca_components/pca_extent, computed once in main() right after
# the scaler is fit at task 0) so every task/checkpoint's plot is directly
# comparable. In THIS file, two checkpoints get captured per task where
# unlearning actually ran: "pre_unlearn" (right after CL training, before
# the forget/unlearn phase) and "post_unlearn" (right after
# unlearn_teacher_guided returns) -- isolating what unlearning itself does
# to the boundary from what ordinary CL training did. Tasks where
# unlearning didn't run (task 0, or the forget set came back empty) get
# just the one "pre_unlearn"-labeled snapshot.
# ---------------------------------------------------------------------------
...
def main():
    ...
    for t, task in enumerate(tasks):
        ...
        if t == 0:
            ...
            # boundary plot for the rest of the run. See
            # plot_decision_boundary's module-level comment for why.
        ...
        # unlearning actually runs for t>0 is decided further below (depends
        # on pc_result/forget_idx), so this is captured unconditionally here
        # and labeled "pre_unlearn" for every t>0 regardless of outcome --
        # if unlearning ends up skipped this task, it's simply the only
        # snapshot for that task, which is itself informative.
        ...
        if t == 0:
            ...
            if pc_result is None:
                ...
                if len(forget_idx) == 0:
                    ...
                    # arrays) so the two are directly comparable: this pair is
                    # what isolates unlearning's own effect on the boundary.
                    boundary_panels.append(plot_decision_boundary(
                        model, DEVICE, pca_mean, pca_components, pca_extent, mal_label,
                        Xtr.numpy(), category, X_test_scaled_np, category_test,
